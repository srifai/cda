...
def tile_raster_images(X, img_shape, tile_shape,tile_spacing = (0,0),
  ...
  if isinstance(X, tuple):
      ...
      if output_pixel_vals:
          ...
          out_array = numpy.zeros((out_shape[0], out_shape[1], 3), dtype=X.dtype)
...
def view_data(data,name,nb_filters=-1,nb_horizontal=-1,nb_vertical=-1):
    ...
    #name='data_image'                                                                                                                                                                                           
    file_namec = name + '_noncen.png'
    image.save(file_namec)

    return image
    def dump_rgb_filters(self,layer_index,layer_sub_index,epoch,image_size,key_word="",filter_is_line=0):


        if(filter_is_line==0):
            nb_filters=self.network.layers[layer_index][layer_sub_index].get_shape()[1]
            nb_horizontal =int( numpy.sqrt(nb_filters) )
            nb_vertical = int( nb_filters/nb_horizontal )
            matrix =self.network.layers[layer_index][layer_sub_index].get_params()[2].value
            size=image_size[0]*image_size[1]
            R=matrix[0:size,:]
            G=matrix[size:(size*2),:]
            B=matrix[(size*2):(size*3),:]

            image_data=tile_raster_images((R.T,G.T,B.T),image_size,(nb_horizontal,nb_vertical),tile_spacing = (2,2),scale_rows_to_unit_interval=True)
            image=PIL.Image.fromarray(image_data)


        file_name = key_word+'W_L'+str(layer_index)+'_l'+str(layer_sub_index)+'_epoch_'+str(epoch)+'.png'
        file_name=os.path.join(self.paths['filters'],file_name)
